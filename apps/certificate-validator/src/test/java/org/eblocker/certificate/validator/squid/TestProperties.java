...
class TestProperties {
    ...
    static String getCaCertificatesFilePath() {
        return LocalCertificateAuthority.INSTANCE.rootPath.toString();
    }
    ...
    static Properties createTestProperties(boolean readerConcurrentFormat) {
        try {
            ...
            java.security.Security.addProvider(new CertificateValidatorProvider());
            properties.setProperty("cache.crl.enabled", "true");
            properties.setProperty("cache.crl.file", LocalCertificateAuthority.INSTANCE.rootPath.resolveSibling("crl.cache").toString());
            properties.setProperty("validator.sun.preferCrls", "true");
