...
function templateCache(done) {
    return gulp.parallel(config.modules.map(module => function cacheModule() {
...
/**
 * Provides a content revision for the language files. We use the same number for both 'en' and 'de' file. This is
 ...
 */
function getLocaleRevision(module) {
    const localePath = path.join(config.src, 'locale');
    const files = fs.readdirSync(localePath)
        .filter(name => name.startsWith('lang-' + module + '-') && name.endsWith('.json'))
        .map(name => ({name: name, content: fs.readFileSync(path.join(localePath, name))}));
    return require('./build-tools/locale-revision')(files);
}
...
function copyAndRenameLangFiles(module, obj) {
    let newFileName = '';
    let pathOnly = '';
    const revision = getLocaleRevision(module);
    return gulp
    // .src(config.build + 'locale/lang-' + module +  '*.json')
        .src(config.src + 'locale/lang-' + module +  '*.json')
        .pipe(jsonminify())
        .pipe(gulpTap(function (file, t) {
            ...
            pathOnly = file.path.split(path.basename(file.path))[0];
            newFileName = getCleanFileaName(path.basename(file.path), revision);
            // ** rename file name and path
            file.path = pathOnly + newFileName;
...
function setLangFilenames(done) {
    const modules = config.modules.concat('shared');
    return gulp.parallel(modules.map(module => function copyModuleLocale() {
        return copyAndRenameLangFiles(module, langObj);
    }))(done);
}
...
// Standalone tests also need the generated locale module and template caches.
exports.test = gulp.series(setLangFilenames, writeDown, templateCache, test);
...
function generateAndWriteJsLicenses(done) {
    ...
    return gulp.series(generateJavaScriptLicenseCSV, convertLicenseCSVtoJSON, createJavaScriptLicenseHTML)(done);
}
    ...
    if (typeof (msg) === 'object') {
        for (let item in msg) {
            if (msg.hasOwnProperty(item)) {
                console.log(ansiColors.blue(msg[item]));
            }
        }
    } else {
        console.log(ansiColors.blue(msg));
    }
}
...
function startTests(singleRun, done) {
    require('./build-tools/run-karma')({
        configFile: __dirname + '/karma.conf.js',
        nodeServer: config.nodeServer,
        singleRun: singleRun,
        startServers: args.startServers
    }, done);
}
