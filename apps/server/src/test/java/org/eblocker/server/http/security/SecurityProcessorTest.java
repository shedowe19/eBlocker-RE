...
public class SecurityProcessorTest {
    ...
    public void validToken() {
        ...
        Mockito.verify(request).putAttachment(Mockito.eq(SecurityProcessor.TOKEN_INFO_ATTACHMENT), Mockito.any(TokenInfo.class));
    }

    @Test
    public void attachesVerifiedTokenToBothActiveRenewalRoutes() {
        Mockito.when(request.getHeader(HttpHeaderNames.AUTHORIZATION.toString())).thenReturn("Bearer 1234");
        for (AppContext context : new AppContext[] { AppContext.ADMINCONSOLE, AppContext.ADMINDASHBOARD }) {
            Mockito.when(route.getName()).thenReturn(context.name().toLowerCase(java.util.Locale.ROOT) + ".authentication.renew.route");
            TokenInfo token = new TokenInfo(context, 1000, 1000, true);
            Mockito.when(tokenHandler.verifyToken("1234")).thenReturn(token);
            processor.process(request);
            Mockito.verify(request).putAttachment(SecurityProcessor.TOKEN_INFO_ATTACHMENT, token);
        }
    }
