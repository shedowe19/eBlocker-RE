...
public class UserServiceInitTest {
    ...
    private final Map<Integer, UserModule> stored = new HashMap<>();
    ...
    public void setup() {
        ...
        when(dataSource.nextId(UserModule.class)).thenReturn(NEW.getId());
        for (UserModule user : Arrays.asList(ALICE, BOB, SYSTEM, LOCKED)) stored.put(user.getId(), user);
        when(dataSource.get(Mockito.eq(UserModule.class), anyInt())).thenAnswer(call -> stored.get(call.getArgument(1)));
        when(dataSource.compareAndSetUser(Mockito.nullable(UserModule.class), any(UserModule.class))).thenAnswer(call -> {
            UserModule next = call.getArgument(1);
    ...
    private void doTest(
        ...
        if (expectedDefaultSystemUser != currentDefaultSystemUser) {
            ...
            verify(dataSource).compareAndSetUser(Mockito.isNull(), Mockito.argThat(user -> user.getId().equals(NEW.getId()) && device.getId().equals(user.getName())));
            verify(dataSource).compareAndSetUser(Mockito.notNull(), Mockito.argThat(user -> user.getId().equals(NEW.getId())));
            verify(dataSource).get(UserModule.class, NEW.getId());
            verify(dataSource).get(org.eblocker.server.common.data.UserProfileModule.class, DefaultEntities.PARENTAL_CONTROL_DEFAULT_PROFILE_ID);
            verify(dataSource).delete(UserModule.class, SYSTEM.getId());
            verify(dashboardService, Mockito.times(2)).getNewDashboardCardColumns(UserRole.OTHER);
        }
        if (expectedDefaultSystemUser != currentDefaultSystemUser ||
