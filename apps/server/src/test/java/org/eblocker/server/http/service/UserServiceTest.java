...
public class UserServiceTest {
    ...
    public void setup() throws IOException {
        ...
        Mockito.when(dataSource.get(Mockito.eq(UserModule.class), Mockito.anyInt())).thenAnswer(invocation ->
                users.stream().filter(user -> user.getId().equals(invocation.getArgument(1))).findFirst().map(this::copyUser).orElse(null));
        Mockito.when(dataSource.compareAndSetUser(Mockito.nullable(UserModule.class), Mockito.any(UserModule.class))).thenAnswer(invocation -> {
            UserModule next = invocation.getArgument(1);
            for (int i = 0; i < users.size(); i++) {
                if (users.get(i).getId().equals(next.getId())) {
                    if (invocation.getArgument(0) == null) return false;
                    users.set(i, copyUser(next));
                    return true;
                }
            }
            if (invocation.getArgument(0) != null) return false;
            users.add(copyUser(next));
            return true;
        });
    ...
    public void testUpdateUser() {
        ...
        Mockito.verify(dataSource).compareAndSetUser(Mockito.any(UserModule.class), Mockito.argThat(saved -> saved.getId().equals(user.getId())));
    ...
    public void testUpdateSystemUser() {
        ...
        Mockito.verify(dataSource).compareAndSetUser(Mockito.any(UserModule.class), Mockito.same(updatedUser));
    }
