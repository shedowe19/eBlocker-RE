...
public class EblockerDnsServer {
    ...
    public synchronized List<LocalDnsRecord> setLocalDnsRecords(List<LocalDnsRecord> localDnsRecords) {
    ...
    public synchronized void refreshLocalDnsRecords() {
        setLocalDnsRecords(getConfig().getLocalDnsRecords());
    }

    public synchronized void setFilteredPeers(Set<IpAddress> peerIps, Set<IpAddress> peerDefaultAllowIps) {
    ...
    private synchronized void updateResolverByIpMapping() {
    ...
    public synchronized void setDnsCustomResolver() {
        DnsServerConfig config = getConfig();
