...
public class NetworkConfiguration {
    ...
    private NetworkConfiguration pendingConfiguration;

    public NetworkConfiguration() { }

    /** Copy settings and observations, excluding revision/pending response metadata. */
    public NetworkConfiguration(NetworkConfiguration source) {
        automatic = source.automatic; expertMode = source.expertMode; dhcp = source.dhcp;
        dnsServer = source.dnsServer; globalIp6AddressAvailable = source.globalIp6AddressAvailable;
        ipAddress = source.ipAddress; networkMask = source.networkMask; vpnIpAddress = source.vpnIpAddress;
        gateway = source.gateway; nameServerPrimary = source.nameServerPrimary; nameServerSecondary = source.nameServerSecondary;
        dhcpRangeFirst = source.dhcpRangeFirst; dhcpRangeLast = source.dhcpRangeLast; ipFixedByDefault = source.ipFixedByDefault;
        rebootNecessary = source.rebootNecessary; advisedNameServer = source.advisedNameServer; dhcpLeaseTime = source.dhcpLeaseTime;
    }
    ...
    @JsonProperty(access = JsonProperty.Access.READ_ONLY)
    public boolean isPendingReboot() { return pendingReboot; }
    public void setPendingReboot(boolean pendingReboot) { this.pendingReboot = pendingReboot; }
    @JsonProperty(access = JsonProperty.Access.READ_ONLY)
    public NetworkConfiguration getPendingConfiguration() { return pendingConfiguration; }
    public void setPendingConfiguration(NetworkConfiguration configuration) { pendingConfiguration = configuration; }
