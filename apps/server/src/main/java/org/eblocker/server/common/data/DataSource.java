...
public interface DataSource {
    ...
    /** Update only a user's PIN if the stored PIN still matches; never overwrite other fields. */
    boolean compareAndSetUserPin(int userId, byte[] expectedPin, byte[] replacementPin);

    /** Replace a freshly read user only if its complete persisted JSON remains unchanged; null expected creates only if absent. */
