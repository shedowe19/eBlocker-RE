...
public class ParentalControlUsageService {
    ...
    public UserProfileModule addBonusTimeForToday(int profileId, int min) {
        return parentalControlService.changeProfileSettings(profileId, profile -> {
            BonusTimeUsage currentBonus = profile.getBonusTimeUsage();
            LocalDateTime now = LocalDateTime.now(clock);
            long previous = currentBonus != null && currentBonus.getDateTime() != null &&
                    now.toLocalDate().equals(currentBonus.getDateTime().toLocalDate()) &&
                    currentBonus.getBonusMinutes() != null ? currentBonus.getBonusMinutes() : 0;
            // Negative deltas deliberately remain supported for withdrawing previously granted time.
            // Calculate in long so an overflowing positive grant cannot silently remove the bonus.
            long total = Math.max(0L, previous + min);
            if (total > Integer.MAX_VALUE) {
                throw new BadRequestException("Bonus time exceeds the supported range");
            }
            profile.setBonusTimeUsage(new BonusTimeUsage(now, (int) total));
        });
    }
    ...
    private Duration getUsageLimit(UserModule user, LocalDate today) {
        ...
        Integer dailyLimit = profile.getMaxUsageTimeByDay().get(today.getDayOfWeek());
        Long maxUsage = dailyLimit == null ? null : dailyLimit.longValue();
        BonusTimeUsage bonusTimeUsage = profile.getBonusTimeUsage();
        if (bonusTimeUsage != null && isToday(bonusTimeUsage.getDateTime())) {
            maxUsage = maxUsage == null ? bonusTimeUsage.getBonusMinutes().longValue() : maxUsage + bonusTimeUsage.getBonusMinutes();
        }
