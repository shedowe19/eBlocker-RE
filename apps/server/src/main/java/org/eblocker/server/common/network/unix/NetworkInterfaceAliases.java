...
public class NetworkInterfaceAliases {
    ...
    public NetworkInterfaceAliases(@Named("network.interface.name") String interfaceName,
        ...
        this(interfaceName, aliasMin, aliasMax, addAliasScript, removeAliasScript, scriptRunner,
                findAliases(interfaceName));
    }

    NetworkInterfaceAliases(String interfaceName, int aliasMin, int aliasMax, String addAliasScript,
                            String removeAliasScript, ScriptRunner scriptRunner, Collection<String> existingAliases) {
        ...
        removeStaleAliases(existingAliases);
    ...
    private static Collection<String> findAliases(String interfaceName) {
        try {
            NetworkInterface networkInterface = NetworkInterface.getByName(interfaceName);
            if (networkInterface != null) {
                return Collections.list(networkInterface.getSubInterfaces()).stream()
                        .map(NetworkInterface::getName).collect(java.util.stream.Collectors.toList());
            }
    ...
    private void removeStaleAliases(Collection<String> aliases) {
        for (String alias : aliases) {
            ...
            try {
                ...
                logger.error("Interrupted while removing stale aliases", e);
                Thread.currentThread().interrupt();
                return;
            }
        }
    ...
    private boolean isManagedAlias(String alias) {
        Pattern pattern = Pattern.compile(Pattern.quote(interfaceName) + ":(\\d+)");
