...
public class DomainBlockingService {
    ...
    private volatile boolean sslServiceInitialized;
    private final AtomicLong filteredDevicesVersion = new AtomicLong();
    ...
    private void updateFilteredDevices() {
        ...
        long snapshotVersion = filteredDevicesVersion.incrementAndGet();
        ...
        // Never acquire DNS while holding this service's monitor: DNS callbacks can enter
        // this service, including through DeviceService.refresh(). The DNS writer monitor
        // serializes publication with resolver/record changes and enabled-state transitions.
        synchronized (eblockerDnsServer) {
            if (snapshotVersion != filteredDevicesVersion.get()) return;
