...
public class NetworkStateMachine {
    ...
    private String configurationRevision = java.util.UUID.randomUUID().toString();
    private PendingNetworkConfiguration pendingConfiguration;
    ...
    public NetworkStateMachine(NetworkServices services,
        ...
        this(services, dataSource, dnsEnableByDefaultChecker, eventLogger, ipSets, sslService, dnsServer, ip6PrefixMonitor, bootId());
    }

    NetworkStateMachine(NetworkServices services, DataSource dataSource, DnsEnableByDefaultChecker dnsEnableByDefaultChecker,
                        EventLogger eventLogger, IpSets ipSets, SslService sslService, EblockerDnsServer dnsServer,
                        Ip6PrefixMonitor ip6PrefixMonitor, String bootId) {
        this.bootId = bootId;
    ...
    public synchronized boolean updateConfiguration(NetworkConfiguration networkConfiguration) {
        loadPendingConfiguration();
        if (pendingConfiguration != null) throw new ConflictException("error.network.rebootPending");
        // Includes legacy controllers, backup restore and DNS status writers.
        // Rotate before any side effect: a partial failure invalidates old snapshots too.
        configurationRevision = java.util.UUID.randomUUID().toString();
        NetworkConfiguration currentNetworkConfiguration = services.getCurrentNetworkConfiguration();
        boolean rebootNeeded = false;
        PendingNetworkConfiguration journal = new PendingNetworkConfiguration(bootId, networkConfiguration);
        if (dataSource.save(journal) == null) throw new IllegalStateException("Could not persist pending network configuration");
        pendingConfiguration = journal;
        ...
        if (!rebootNeeded) {
            dataSource.delete(PendingNetworkConfiguration.class);
    ...
    /** Runtime fields and desired state are observed under the same monitor as every writer. */
    public synchronized NetworkConfiguration getConfigurationSnapshot() {
        loadPendingConfiguration();
        NetworkConfiguration result = new NetworkConfiguration(services.getCurrentNetworkConfiguration());
        result.setRevision(configurationRevision);
        result.setPendingReboot(pendingConfiguration != null);
        result.setRebootNecessary(pendingConfiguration != null);
        result.setPendingConfiguration(pendingConfiguration == null ? null : pendingConfiguration.configuration());
        return result;
    }

    public synchronized void requireConfigurationRevision(String expected) {
        loadPendingConfiguration();
        if (expected == null || !expected.equals(configurationRevision)) throw new ConflictException("error.network.settingsChanged");
        if (pendingConfiguration != null) throw new ConflictException("error.network.rebootPending");
    }

    private void loadPendingConfiguration() {
        if (pendingLoaded) return;
        PendingNetworkConfiguration stored = dataSource.get(PendingNetworkConfiguration.class);
        if (stored != null && bootId != null && stored.bootId() != null && !bootId.equals(stored.bootId())) {
            // Only an actual kernel reboot consumes the pending reboot, never a Java restart.
            dataSource.delete(PendingNetworkConfiguration.class);
            stored = null;
        }
        pendingConfiguration = stored;
        pendingLoaded = true;
    }

    private static String bootId() {
        try {
            String id = java.nio.file.Files.readString(java.nio.file.Path.of("/proc/sys/kernel/random/boot_id")).trim();
            return id.matches("[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}") ? id : null;
        } catch (java.io.IOException | SecurityException e) {
            return null; // Unknown boot identity must never discard pending desired state.
        }
    }
     ...
     */
    public synchronized void deviceStateChanged() {
        NetworkStateId currentState = getCurrentNetworkState().getId();
        services.enableFirewall(shouldMasquerade(currentState), isSSLEnabled(), isOpenVpnServerEnabled(), ipSets.isSupportedByOperatingSystem());
        if (currentState == NetworkStateId.LOCAL_DHCP) {
            loadPendingConfiguration();
            services.configureDhcpServer(pendingConfiguration == null ? services.getCurrentNetworkConfiguration() : pendingConfiguration.configuration());
        }
    }
