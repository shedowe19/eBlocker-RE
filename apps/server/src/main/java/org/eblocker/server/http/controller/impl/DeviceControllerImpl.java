...
import org.eblocker.server.http.controller.DeviceSettingsPatch;
...
public class DeviceControllerImpl implements DeviceController {
    ...
    public Object deleteDevice(Request request, Response response) {
        synchronized (pauseDeviceController) {
    ...
    @Override
    public Device updateDeviceSettings(Request request, Response response) {
        DeviceSettingsPatch patch = DeviceSettingsPatch.parse(request.getBody().toString(StandardCharsets.UTF_8));
        String deviceId = request.getHeader("deviceId");
        // The pause scheduler uses this monitor too. Keep its state stable while applying settings,
        // and serialize concurrent PATCH/PUT requests through the same updateDevice lock below.
        synchronized (pauseDeviceController) {
            if ((Device.ID_PREFIX + networkInterfaceWrapper.getHardwareAddressHex()).equals(deviceId)) {
                throw new ForbiddenException("error.device.settings.appliance");
            }
            Device current = deviceService.getDeviceById(deviceId);
            if (current == null) {
                throw new NotFoundException("error.device.settings.notFound");
            }
            if (current.isEblocker() || current.isGateway()) {
                throw new ForbiddenException("error.device.settings.infrastructure");
            }
            if (current.isPaused() && patch.changesEnabled(current)) {
                throw new ConflictException("error.device.settings.paused");
            }
            Device updated = patch.applyTo(current);
            // Existing validation, persistence, listeners, firewall and VPN handling stay authoritative.
            // Network changes cannot be rolled back atomically with persistence: propagate failures,
            // and never restore a stale snapshot over changes made by another service.
            updateDeviceWithPauseStateLocked(updated, patch.changesEnabled(current));
            Device result = deviceService.getDeviceById(deviceId);
            if (result == null) {
                throw new NotFoundException("error.device.settings.notFound");
            }
            Device responseDevice = DeviceSettingsPatch.responseCopy(result,
                    result.getIpAddresses().contains(ControllerUtils.getRequestIPAddress(request)));
            deviceService.setOnlineStatus(responseDevice);
            return responseDevice;
        }
    }

    @Override
    public Device assignUser(String deviceId, int userId) {
        synchronized (pauseDeviceController) {
            Device current = deviceService.getDeviceById(deviceId);
            if (current == null) throw new NotFoundException("error.device.settings.notFound");
            if (current.isEblocker() || current.isGateway()
                    || (Device.ID_PREFIX + networkInterfaceWrapper.getHardwareAddressHex()).equals(deviceId)) {
                throw new ForbiddenException("error.device.settings.infrastructure");
            }
            Device updated = DeviceSettingsPatch.copyOf(current);
            updated.setAssignedUser(userId);
            updated.setOperatingUser(userId);
            updateDeviceWithPauseStateLocked(updated, false);
            Device result = deviceService.getDeviceById(deviceId);
            if (result == null) throw new NotFoundException("error.device.settings.notFound");
            return DeviceSettingsPatch.copyOf(result);
        }
    }
    ...
    private void updateDevice(Device device) {
        synchronized (pauseDeviceController) {
            updateDeviceWithPauseStateLocked(device, true);
        }
    }

    private void updateDeviceWithPauseStateLocked(Device device, boolean reconcileAnonymization) {
        ...
        // A settings PATCH cannot change VPN/Tor selection. Renaming a device or changing
        // filters must not restart/reconfigure its routing or unnecessarily block pause expiry.
        if (!reconcileAnonymization) {
            deviceService.setOnlineStatus(device);
            return;
        }
