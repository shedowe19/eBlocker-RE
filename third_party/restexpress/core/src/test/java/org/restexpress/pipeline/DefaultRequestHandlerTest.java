...
public class DefaultRequestHandlerTest
{
    ...
    @Test
    public void transportSecurityComesFromPipelineNotForwardedHeaders() throws Exception {
        java.util.concurrent.atomic.AtomicReference<Object> secure = new java.util.concurrent.atomic.AtomicReference<>();
        messageHandler.addPreprocessor(request -> secure.set(request.getAttachment("transport.secure")));
        DefaultFullHttpRequest request = new DefaultFullHttpRequest(HttpVersion.HTTP_1_1, HttpMethod.GET, "/unserializedDefault");
        request.headers().set("Host", "device.example");
        request.headers().set("X-Forwarded-Proto", "https");
        pl.fireChannelRead(request);
        assertEquals(Boolean.FALSE, secure.get());
    }

    @Test
    public void transportSecurityRecognizesActualTlsHandler() throws Exception {
        java.util.concurrent.atomic.AtomicReference<Object> secure = new java.util.concurrent.atomic.AtomicReference<>();
        messageHandler.addPreprocessor(request -> secure.set(request.getAttachment("transport.secure")));
        javax.net.ssl.SSLEngine engine = javax.net.ssl.SSLContext.getDefault().createSSLEngine();
        engine.setUseClientMode(false);
        // TLS follows the request handler only in this unit test so plaintext test
        // input reaches it; production has TLS before HTTP decoding as usual.
        pl.addLast("tls", new io.netty.handler.ssl.SslHandler(engine));
        sendGetEvent("/unserializedDefault");
        assertEquals(Boolean.TRUE, secure.get());
    }
...
//		System.out.println(httpResponse.toString());
	...
	throws Exception
	{
		sendGetEvent("/serializedString.json?returnValue=raw%20string");
		assertEquals(0, observer.getExceptionCount());
		assertEquals(1, observer.getReceivedCount());
		assertEquals(1, observer.getCompleteCount());
		assertEquals(1, observer.getSuccessCount());
